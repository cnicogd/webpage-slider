(function () {
    ...
    function k(b, c) {
        function l(a, c, d) {
            ...
            n.find("#" + b + " .cgn-caption").css("visibility", "hidden");
            T();
            ca();
            x(f.play_type);
            da();
        ...
        function e() {
            d("#" + b + "-cgn-loading-mask").remove();
            var a = document.createElement("DIV");
            a.id = b + "-cgn-loading-mask";
        ...
        function h() {
            d("#" + b).show();
            1 == d("#" + b + "-cgn-loading-mask").length && d("#" + b + "-cgn-loading-mask").fadeOut(400, function () {
                d("#" + b + "-cgn-loading-mask").remove();
                O(0);
                U(0)
            })
            ...
            for (g = 0; g < c; g++) {
                ...
                for (var l = 0; 3 > l; l++) {
                    var e = {
                        ...
                        t = d(a.get(g)).find(".cgn-caption-" + l);
        ...
        function r() {
            ...
            X();
            d("#" + b + " .cgn-radios input.cgn-radio, #" +
                b + " .cgn-radios .cgn-radio-label").off("click", H);
            d("#" + b + " .cgn-radios input.cgn-radio, #" + b + " .cgn-radios .cgn-radio-label").off("touchstart", H);
            Y();
            d("#" + b + " .cgn-button").remove();
            d("#" + b + " .cgn-radios").remove();
            d("#" + b + " .cgn-cloned").remove();
            for (d("#" + b + " .cgn-tmp-clone").remove(); 0 < d("#" + b + " .cgn-frame").length;) d("#" + b + " .cgn-slides").unwrap();
            0 < d("#" + b + " .cgn-img-frame").length && d("#" + b + " .cgn-img").unwrap();
            d("#" + b + " .cgn-caption").removeAttr("style");
            d("#" + b + " .cgn-slide").show();
            d("#" +
                b + " .cgn-slide").removeAttr("style");
            d("#" + b + " .cgn-slides").removeAttr("style");
            d("#" + b + " .cgn-img").removeClass("cgn-img");
            d("#" + b + " .cgn-slide").removeClass("cgn-slide cgn-slide-0 cgn-slide-1 cgn-slide-2 cgn-slide-3 cgn-slide-4 cgn-slide-5 cgn-slide-6 cgn-slide-7 cgn-slide-8 cgn-slide-9");
            d("#" + b + " .cgn-slides").removeClass("cgn-slides");
        ...
        function u() {
            ...
            p = 0;
            n.addClass("cgn-slides");
            d("#" + b + " .cgn-slides > li").addClass("cgn-slide");
            d("#" + b + " .cgn-slides > .cgn-slide > img, #" + b + " .cgn-slides > .cgn-slide > a > img").addClass("cgn-img");
            n.find("li").each(function (a) {
                this.addClass("cgn-slide-" + a)
            });
            d("#" + b).wrapInner("<div class='cgn-frame'></div>");
            n = d("#" + b + " ol.cgn-slides");
            d("#" + b + " .cgn-img").wrap("<div class='cgn-img-frame'></div>")
        }

        function m() {
            return d("#" + b + " .cgn-slide").length
        }

        function x(a) {
            f.play_type = a;
            Y();
            d("#" + b + " .cgn-pause-button").remove();
            f.pause_button && "manual" != f.play_type && (n.after("<div class='cgn-pause-button'>&nbsp;</div>"),
                C = d("#" + b + " .cgn-pause-button"), C.on("click", I), C.on("touchstart", I));
            v()
        }

        function v() {
            A = !0;
            d("#" + b + " .cgn-pause-button").removeClass("cgn-play");
        ...
        function g() {
            ...
            clearTimeout(B);
            B = null;
            d("#" + b + " .cgn-pause-button").addClass("cgn-play")
        }
        ...
        function T() {
            f.buttons && (n.after("<div class='cgn-button cgn-button-prev'>&nbsp;</div><div class='cgn-button cgn-button-next'>&nbsp;</div>"), E = d("#" + b + " .cgn-button-prev"), D = d("#" + b + " .cgn-button-next"), E.on("click", J), E.on("touchstart", J), D.on("click", K), D.on("touchstart", K))
        }

        function X() {
            d("#" + b + " .cgn-button-prev").off("click", J);
            d("#" + b + " .cgn-button-prev").off("touchstart", J);
            d("#" + b + " .cgn-button-next").off("click", K);
            d("#" + b + " .cgn-button-next").off("touchstart", K)
        }
        ...
        function ca() {
            if (f.radios) {
                d("#" + b).append("<ul class='cgn-radios'></ul>");
                z = d("#" + b + " .cgn-radios");
                "thumbnail" == f.radio_type && z.addClass("cgn-radios-as-thumbnails");
                for (var a = 0; a < m(); a++) 0 == a ? z.append("<li class='cgn-radio-" + a + " active'><input type='radio' name='cgn-radio' class='cgn-radio' id='cgn-radio-" + a +
                    "' checked='checked' /><label class='cgn-radio-label' for='cgn-radio-" + a + "'></label></li>") : z.append("<li class='cgn-radio-" + a + "'><input type='radio' name='cgn-radio' class='cgn-radio' id='cgn-radio-" + a + "' /><label class='cgn-radio-label' for='cgn-radio-" + a + "'></label></li>");
                d("#" + b + " .cgn-radios input.cgn-radio, #" + b + " .cgn-radios .cgn-radio-label").on("click", H);
                d("#" + b + " .cgn-radios input.cgn-radio, #" + b + " .cgn-radios .cgn-radio-label").on("touchstart", H)
            }
        }
        ...
        function Z(a) {
            f.radios && (d("#" + b + " .cgn-radios li").removeClass("active"), a = d("#" + b + " .cgn-radios li").get(a), d(a).addClass("active"), d(a).find("input").attr("checked", "checked"))
        }

        function Y() {
            d("#" + b + " .cgn-pause-button").off("click", I);
            d("#" + b + " .cgn-pause-button").off("touchstart", I)
        }
        ...
        function S(a, c) {
            if (1 == c ||
                a != f.transition_type) {
                f.transition_type = a;
                d("#" + b + " .cgn-slide").removeClass("cgn-zoom-in");
                d("#" + b + " .cgn-slide").show();
                var g = m();
                n.css("width", 100 * g + "%");
                n.find(".cgn-slide").each(function (a) {
                    a = N * a + "%";
        ...
        function ba(a, c, g, l) {
            d("#" + b + " li.cgn-slide").removeClass("cgn-zoom-in");
            var t = d("#" + b + " li.cgn-slide-" + a),
                e = n.clone();
            e.addClass("cgn-slides-clone");
            var h = c / (m() - 1),
                h = q.getOffsetsByRatios([h, 0]);
            e.css("transform", "translateX(" + h[0] + "px)");
            e.insertAfter(n.el);
            g && t.addClass("cgn-zoom-in");
            g = e.get(0);
            n.fadeInto(g, 2 * f.transition_duration, function () {
                q.setStep(c + 1, 1, !0);
                d("#" +
                    b + " .cgn-slides-clone").remove();
                d("#" + b + " .cgn-slides").show();
                d("#" + b + " .cgn-slides").css("opacity", null);
                L(a, c, !0, l)
            })
        }
        ...
        function O(a) {
            W();
            "none" != f.image_fx && ("zoompan" == f.image_fx ? d("#" + b + " .cgn-slide-" + a + " .cgn-img-frame").addClass("cgn-zoom-pan") : "zoomrotate" == f.image_fx && d("#" + b + " .cgn-slide-" +
                a + " .cgn-img-frame").addClass("cgn-zoom-rotate"))
        }

        function W() {
            d("#" + b + " .cgn-slide .cgn-img-frame").removeClass("cgn-zoom-pan");
            d("#" + b + " .cgn-slide .cgn-img-frame").removeClass("cgn-zoom-rotate")
        }

        function U(a) {
            n.find(".cgn-caption").css("visibility", "hidden");
            n.find(".cgn-caption").removeClass("cgn-caption-anim");
            R(a, 0);
            R(a, 1);
            R(a, 2)
        }
        ...
        function R(a, b) {
            f.captions[a] && 1 == f.captions[a][b].enable && setTimeout(function () {
                n.find(".cgn-slide-" + a + " .cgn-caption-" + b).css("visibility", "visible");
                n.find(".cgn-slide-" +
                    a + " .cgn-caption-" + b).addClass("cgn-caption-anim")
        ...
        this.enableButtons = function (a) {
            !0 !== a || f.buttons || (X(), d("#" + b + " .cgn-button").remove(), T())
        };
        this.enableRadios = function (a) {};
        this.setRadioType = function (a) {
            "thumbnail" == a ? d("#" + b + " .cgn-radios").addClass("cgn-radios-as-thumbnails") : d("#" +
                b + " .cgn-radios").removeClass("cgn-radios-as-thumbnails")
        };
    ...
    window.cgn.Slider = k;
    window.cgn.Config = window.cgn.Config || {};
    window.cgn.instances = [];
    d.ready(function () {
        if (!0 !== window.cgn.Config.no_instantiation)
            for (var b = d(".cgn-slider"), c = 0; c < b.length; c++) {
                var e = b.get(c);
                e.id = e.id || "cgn-slider-" + c;
                window.cgn.instances.push(new k(e.id, {}))
            }
    })
})();
(function (e, d) {
    ...
    d.prototype = {
        ...
        onHandleMouseDown: function (a) {
            a.target && "A" == a.target.tagName && 0 <= a.target.className.search(/cgn-caption/) && (document.location = a.target.href);
            h.refresh(a);
            l(a);
            M(a);
            this.activity = !1;
        ...
        onWrapperMouseDown: function (a) {
            a.target && 0 <= a.target.className.search(/cgn-(button|radio|caption)/) || (h.refresh(a), l(a), this.startTap())
        },
        ...
        onDocumentMouseUp: function (a) {
            ...
            this.stopTap();
            a.target &&
                0 <= a.target.className.search(/cgn-(button|radio|caption)/) || this.options.afterSwipeCallback()
        },
        ...
        onHandleClick: function (a) {
            a.target && 0 <= a.target.className.search(/cgn-(button|radio)/) || !this.activity || (l(a), M(a))
        },
